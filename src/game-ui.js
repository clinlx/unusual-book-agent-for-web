...
const GameUI = (() => {
  ...
  async function boot() {
    ...
    document.addEventListener('submit',e=>{e.preventDefault();attempt(async()=>{
      ...
      if(e.target.id==='action-form'){if(state.running)return;if(!apiKeyWarning())return;const flags=GameData.actorState(GameApp.player().info);if(!flags.alive)return;if(!draft.trim()||!flags.enabled){if(confirm(flags.enabled?'本回合不采取主动行动，确定空过？':'失去意识，等待局势发展并推进一轮？'))await GameApp.skip();return;}clearTimeout(draftTimer);const text=draft,count=(state.active.events||[]).filter(x=>x.type==='player').length,input=e.target.querySelector('#action-input');draft='';if(input){input.value='';refreshInput();}try{await GameApp.send(text);}catch(error){if((state.active.events||[]).filter(x=>x.type==='player').length===count){draft=text;await GameApp.saveDraft(text);}render();const restored=document.getElementById('action-input');if(restored&&restored.value!==draft){restored.value=draft;refreshInput();}throw error;}}
