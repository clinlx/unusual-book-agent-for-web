...
const GameUI = (() => {
  ...
  let playViewportAnchor=null,debugAutoSynced=false,pendingDebugSync=false;
  ...
  function capturePlayViewport(){
    if(state?.mode==='debug'||!state?.active)return playViewportAnchor;
    const h=document.getElementById('history');if(!h)return playViewportAnchor;
    const nodes=[...h.children].filter(n=>n?.dataset?.eventId);if(!nodes.length)return playViewportAnchor;
    const hr=h.getBoundingClientRect(),focusRatio=.38,focusY=hr.top+hr.height*focusRatio;
    let target=nodes.find(n=>{const r=n.getBoundingClientRect();return r.top<=focusY&&r.bottom>=focusY;});
    if(!target)target=nodes.reduce((best,n)=>{const r=n.getBoundingClientRect(),distance=Math.abs((r.top+r.bottom)/2-focusY);return !best||distance<best.distance?{node:n,distance}:best;},null)?.node;
    if(!target)return playViewportAnchor;
    const r=target.getBoundingClientRect(),inside=r.height?Math.max(0,Math.min(1,(focusY-r.top)/r.height)):0;
    playViewportAnchor={saveId:state.active.id,eventId:target.dataset.eventId,round:Number(target.dataset.eventRound)||null,inside,focusRatio};
    return playViewportAnchor;
  }
  function syncDebugViewport({silent=false}={}){
    if(state?.mode!=='debug'||!state?.active)return false;
    const h=document.getElementById('history'),anchor=playViewportAnchor;
    if(!h||!anchor||anchor.saveId!==state.active.id){if(!silent)toast('还没有可同步的游戏视图位置');return false;}
    const nodes=[...h.children].filter(n=>n?.dataset?.eventId);
    let target=nodes.find(n=>n.dataset.eventId===anchor.eventId);
    if(!target&&anchor.round!==null)target=nodes.find(n=>Number(n.dataset.eventRound)===anchor.round);
    if(!target){if(!silent)toast('没有找到对应的回合元素');return false;}
    const hr=h.getBoundingClientRect(),r=target.getBoundingClientRect(),nodeTop=r.top-hr.top+h.scrollTop;
    const desired=nodeTop+r.height*(anchor.inside??0)-h.clientHeight*(anchor.focusRatio??.38);
    const max=Math.max(0,h.scrollHeight-h.clientHeight),top=Math.max(0,Math.min(max,desired));
    h.scrollTo({top,behavior:'auto'});lastScroll=top;scrollAmount=0;
    target.classList.add('viewport-sync-hit');setTimeout(()=>target.classList.remove('viewport-sync-hit'),650);
    if(!silent)toast('已同步到游戏视图附近');
    return true;
  }
  ...
  function render() {
    ...
    if(next&&s.mode==='debug'&&pendingDebugSync){pendingDebugSync=false;requestAnimationFrame(()=>{if(state?.mode==='debug'&&state.active?.id===a?.id)syncDebugViewport({silent:true});});}
    else if(next&&s.mode!=='debug')requestAnimationFrame(()=>{if(state?.mode!=='debug'&&state.active?.id===a?.id)capturePlayViewport();});
  ...
  function settingsModal() {
    ...
    const optionalField=(name,label,min,max,step='any',placeholder='默认')=>`<label>${label}<input name="${name}" type="number" value="${esc(s[name]??'')}" min="${min}" max="${max}" step="${step}" placeholder="${placeholder}"></label>`;
    const apiHelp=`<div class="api-options-help" id="api-options-help" hidden><p><strong>采样参数</strong>：Temperature 控制随机性；Top P 控制 nucleus sampling 范围；Frequency Penalty 降低重复用词；Presence Penalty 鼓励引入新内容；Seed 在服务商支持时用于尽量复现结果。</p><p><strong>自定义请求包体</strong>会在标准请求组装完成后按字段合并：新字段会追加，同名字段会覆盖；嵌套对象递归合并，数组和普通值整体覆盖。可用于服务商私有参数。</p><pre>{\n  "top_p": 0.92,\n  "stream_options": {"include_usage": true},\n  "extra_body": {\n    "chat_template_kwargs": {"enable_thinking": true}\n  }\n}</pre><p>例如填写 <code>{"temperature":0.3}</code> 会覆盖上方 Temperature；填写 <code>{"foo":"bar"}</code> 会新增字段。自定义内容也可以覆盖 model、messages、tools、stream 等核心字段，使用这些字段时请确认服务端行为。</p></div>`;
    const api=`<div class="api-settings-heading"><span>请求参数</span>${button('api-options-help','i','class="settings-info-button" aria-expanded="false" aria-controls="api-options-help" title="参数说明与示例"')}</div><div class="settings-grid">${field('baseUrl','API 地址','url')}${model}${apiKey}${field('temperature','Temperature','number',0.7)}${optionalField('topP','Top P',0,1)}${optionalField('frequencyPenalty','Frequency Penalty',-2,2)}${optionalField('presencePenalty','Presence Penalty',-2,2)}${optionalField('seed','Seed',-2147483648,2147483647,'1')}${field('maxContextK','上下文上限（K tokens）','number',240)}${field('maxOutputTokens','最大输出 tokens','number',16384)}${field('maxToolLoops','每轮最大模型调用次数','number',60)}${field('httpTimeoutSeconds','请求超时（秒）','number',180)}${field('maxRetries','重试次数','number',2)}<label>思考强度<select name="reasoningEffort">${['none','low','high','xhigh','max'].map(v=>`<option ${s.reasoningEffort===v?'selected':''}>${v}</option>`).join('')}</select></label><label class="check-label stream-setting"><input name="stream" type="checkbox" ${s.stream!==false?'checked':''}><span>流式输出<small>用于缓解长时间输出文本的超时问题；不一定能使故事输出本身变为流式。</small></span></label></div>${apiHelp}<label class="custom-request-body-setting"><span>自定义请求包体 JSON</span><textarea name="customRequestBody" class="custom-request-body" spellcheck="false" placeholder='{"top_p":0.92,"stream_options":{"include_usage":true}}'>${esc(s.customRequestBody||'')}</textarea><small>留空则不追加。标准字段先生成，自定义 JSON 后合并，因此同名字段以这里为准。</small></label>`;
  ...
  function diceSpecRows(args={}){
    const out=[];for(const [label,value] of Object.entries(args.dice_dict||{})){
      ...
      else if(forced)out.push({label,forced:Number(forced[1]),formula,index:0});
  ...
  const isPhysicalDiceSpec=spec=>spec?.constant===undefined&&spec?.forced===undefined&&PHYSICAL_DICE_FACES.has(Number(spec.faces));
  ...
  function fallbackDiceHTML(specs,values){
    ...
    for(let i=0;i<specs.length;i++){
      ...
      if(spec.forced!==undefined){out+=`<div class="manual-force-value" data-spec-index="${i}"><small>SET / FORCE</small><strong>${esc(spec.forced)}</strong><span>${esc(safeDiceLabel(spec.label)||'固定值')}</span></div>`;continue;}
  ...
  async function animateManualDice(event){
    ...
    for(let i=0;i<specs.length;i++){
      const spec=specs[i];if(spec.constant!==undefined||spec.forced!==undefined)continue;
  ...
  async function action(name,el) {
    ...
    if(name==='api-options-help'){const panel=document.getElementById('api-options-help');if(!panel)return;panel.hidden=!panel.hidden;el.setAttribute('aria-expanded',String(!panel.hidden));return;}
    ...
    if(name==='sync-debug-window')return syncDebugViewport();
    if(name==='debug'){closeFileMenu();if(state.mode!=='debug'){capturePlayViewport();if(!debugAutoSynced){debugAutoSynced=true;pendingDebugSync=true;}}return GameApp.setMode('debug');}
    if(name==='play'){closeFileMenu();return GameApp.setMode('play');}
