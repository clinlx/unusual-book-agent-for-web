...
const GameUI = (() => {
  ...
  function importMenu(){
    ...
    showModal('开启新的故事',`<div class="import-choices">${catalog.length?choice('catalog','▤','从列表选择','浏览收录的世界，寻找下一段旅程'):''}${choice('import-url','↗','从链接导入','粘贴 ZIP 链接，载入远方的故事')}${choice('import-file','◇','从文件导入','打开设备中的 ZIP 模组或存档')}</div>`);
  ...
  function settingsModal() {
    ...
    showModal('设置',`<aside class="settings-github" aria-label="GitHub"><div><strong>GitHub</strong><p>View source code or contact the author for feedback.</p></div><a class="github-link" href="https://github.com/clinlx/unusual-book-agent-for-web" target="_blank" rel="noopener noreferrer" aria-label="Open GitHub repository">GitHub ↗</a></aside><form id="settings-form"><div class="settings-grid">${field('baseUrl','API 地址','url')}${field('model','模型')}${field('apiKey','API Key','password')}${field('temperature','Temperature','number',0.8)}${field('maxContextK','上下文上限（K tokens）','number',128)}${field('maxOutputTokens','最大输出 tokens','number',16384)}${field('maxToolLoops','每轮最大模型调用次数','number',60)}${field('httpTimeoutSeconds','请求超时（秒）','number',180)}${field('maxRetries','重试次数','number',2)}<label>思考强度<select name="reasoningEffort">${['none','low','high','xhigh','max'].map(v=>`<option ${s.reasoningEffort===v?'selected':''}>${v}</option>`).join('')}</select></label><label class="check-label"><input name="stream" type="checkbox" ${s.stream!==false?'checked':''}>流式输出</label></div><div class="modal-actions">${button('test-connection','测试连接')}${button('prompts','编辑提示词')}<button class="primary" type="submit">保存设置</button></div><p id="connection-result" role="status"></p></form>`);
  }
